...
IX_BTNode* IX_IndexHandle::GetParent(IX_BTNode * left_node){
    IX_BTNode* node = left_node;
...
RC IX_IndexHandle::InsertEntry(void *pData, const RID &rid, int detail) {
    ...
    while (result == -1) {
        //std::cerr << "non root overflow" << std::endl;
            IX_BTNode * parent = path[level];
            int posAtParent = pathP[level];
            IX_BTNode * left_node= FetchNode(node->GetLeft());
                 
           
        
            if(left_node!=0&&level==(fileHdr.height-1)){
        //std::cerr<<"left_node:"<<left_node<<std::endl;
        //std::cerr<<"left node key:"<<left_node->GetKeysNum()<<std::endl;
        //std::cerr<<"left_P,P:"<<*parent<<parent<<","<<GetParent(left_node)<<std::endl;
            if((left_node->GetKeysNum())<(left_node->GetOrder())&&(parent==GetParent(left_node))){
           //  std::cerr<<"distribute"<<std::endl;
            const void* key = NULL;
            
            int detail=0;
                RC tempKey=node->GetKey(0,(void*&) key);
            //std::cerr<<"GetKey"<<std::endl;
           // std::cerr<<"key:"<<key<<std::endl;
            //std::cerr<<*&tempKey<<std::endl;
                RID tempRid=node->FindAddrBrut(key);
	       // std::cerr<<"GetRid"<<key<<std::endl;
	    int pos_temp=node->FindKeyExact((const void*&) key,node->GetAddr(0));	
		    left_node->InsertNode(key,node->GetAddr(0)); 
           // std::cerr<<"insertLeft"<<*node<<std::endl;
		//node->GetKey(0,(void*&)key);
               	node->DeleteNode(key,pos_temp);
            // std::cerr<<"delete"<<node<<std::endl;
 		//node->GetKey(0,(void*&)key);          
               parent->DeleteNode(NULL, posAtParent);
               result = parent->InsertNode((const void*) left_node->LargestKey(),
                left_node->GetNodeRID());
           //std::cerr<<"change parent"<<key<<std::endl;
	       // std::cerr<<"n:"<<*node<<std::endl;
                node->InsertNode(failedKey,failedRid);
           	
		//std::cerr<<"reinsert"<<*left_node<<std::endl; 
	...
	if(distribute_flag==1)std::cerr<<"distribution happend"<<std::endl;
        // unpin old root page
        ...
        rc = GetThisPage(p, ph);
        if (rc != 0) return IX_PF;
	//std::cerr<<"make new root node"<<std::endl;
        // rc = pfFileHandle->MarkDirty(p);
        // if(rc!=0) return NULL;
